...
[tibia_indx,~] = listdlg('PromptString',{'Select the tibia'}, 'ListString', list_files, 'SelectionMode','multiple');
[talus_indx,~] = listdlg('PromptString',{'Select the talus'}, 'ListString', list_files, 'SelectionMode','multiple');

tibia_files = list_files(tibia_indx)';
talus_files = list_files(talus_indx)';
...
% for m = 1:length(all_files)
    ...
    FileName_tib = char(tibia_files);
    [~,name_tib,ext] = fileparts(FileName_tib);
    disp(name_tib)
        FileName_tal = char(talus_files);
    [~,name_tal,ext] = fileparts(FileName_tal);
    disp(name_tal)
...
%         for n = 1:length(list_bone)
%             if any(string(extract(FileName_tib,list_bone(n))) == string(list_bone(n))) ||...
%                     any(string(extract(FileName_tib,lower(list_bone(n)))) == lower(string(list_bone(n)))) ||...
%                     any(string(extract(FileName_tib,upper(list_bone(n)))) == upper(string(list_bone(n))))
%                 bone_indx = n;
%             end
%         end
%     end
...
%     % the bone name
%     if exist('bone_indx') == 0
%         [bone_indx,~] = listdlg('PromptString', [{strcat('Select which bone this file is:'," ",string(FileName_tib))} {''}], 'ListString', list_bone,'SelectionMode','single');
%     end
% 
...
%         for n = 1:length(list_side_folder)
%             if any(string(extract(FileName_tib,list_side_folder(n))) == string(list_side_folder(n))) ||...
%                     any(string(extract(FileName_tib,lower(list_side_folder(n)))) == lower(string(list_side_folder(n)))) ||...
%                     any(string(extract(FileName_tib,upper(list_side_folder(n)))) == upper(string(list_side_folder(n))))
%                 side_folder_indx = n;
%             end
%         end
%     end
    ...
    elseif exist('side_folder_indx') && side_folder_indx >= 3
        ...
        [side_indx,~] = listdlg('PromptString', [{strcat('Select which side this file is:'," ",string(FileName_tib))} {''}], 'ListString', list_side,'SelectionMode','single');
    end

    %% Load in file based on file type
    if ext == ".k"
        nodes_tib = LoadKFile(FileName_tib);
    elseif ext == ".stl"
        TR_tib = stlread(FileName_tib);
        nodes_tib = TR_tib.Points;
        conlist_tib = TR_tib.ConnectivityList;
        TR_tal = stlread(FileName_tal);
        nodes_tal = TR_tal.Points;
        conlist_tal = TR_tal.ConnectivityList;
    elseif ext == ".particles"
        nodes_tib = load(FileName_tib);
    elseif ext == ".vtk"
        nodes_tib = LoadVTKFile(FileName_tib);
    elseif ext == ".ply"
        ptCloud = pcread(FileName_tib);
        nodes_tib = ptCloud.Location;
    else
...
% 
%     if bone_indx == 1 && bone_coord == 1
%         name_tib = strcat('TN_',name_tib);
%     elseif bone_indx == 1 && bone_coord == 2
%         name_tib = strcat('TTST_',name_tib);
%     end
    
    if length(name_tib) > 31
        name_tib = name_tib(1:31);
    end

    xlfilename = strcat(FolderPathName,'\MDTATT_',FolderName,'.xlsx');
    writematrix(A,xlfilename,'Sheet',name_tib);
%     writecell(B,xlfilename,'Sheet',name_tib,'Range','B1');
    writematrix(C,xlfilename,'Sheet',name_tib,'Range','A5');
%     writematrix(D,xlfilename,'Sheet',name,'Range','B5')
%     writematrix(D,xlfilename,'Sheet',name,'Range','B10')
    writematrix(MDTA,xlfilename,'Sheet',name_tib,'Range','B5');
     writematrix(TT,xlfilename,'Sheet',name_tib,'Range','B6');
     writematrix(TLSA,xlfilename,'Sheet',name_tib,'Range','B7');
